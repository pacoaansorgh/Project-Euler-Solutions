...
-- The 100 50-digit numbers stored in a two-dimensional array. Each number is read from left to right, with every line being its own number.
numbers =  {{3,7,1,0,7,2,8,7,5,3,3,9,0,2,1,0,2,7,9,8,7,9,7,9,9,8,2,2,0,8,3,7,5,9,0,2,4,6,5,1,0,1,3,5,7,4,0,2,5,0},
...
-- Variables: buffer is where we'll add the digits on each column, index is used to know when to start saving the numbers, digits will contain our final answer
buffer = 0
index = 10
digits = {}
...
for x = #numbers[1], 1, -1 do
	-- Add all the numbers in the row we're currently looking at to the buffer
	for y = 1, #numbers do
		buffer = buffer + numbers[y][x]
	end
	-- Now the buffer contains all those numbers, we need to check if we're far along enough to start saving numbers
	if x <= index then
		-- If so, save the last digit of the buffer
		digits[x] = buffer % 10
	end
	-- Now divide the buffer by 10 so we lose the last digit
	buffer = math.floor(buffer/10)
end

io.write(buffer, '\n')
-- At this point we have our 10 digits, but there is still some leftover in our buffer. So we're going to have to empty the buffer digit by digit.
while buffer ~= 0 do
	for i = #digits, 2, -1 do
		digits[i] = digits[i-1]
	end
...
-- Now print our final answer
io.write("The first 10 digits of the sum of these numbers are: ")
for i = 1, #digits do
	io.write(digits[i])
end
