// Find the first 10 digits of the sum of these given numbers.
#include <iostream>

#define NR_AMOUNT 100
#define NR_LENGTH 50
#define DIGITS 10
...
int main(){
    int numbers[NR_AMOUNT][NR_LENGTH] =  {{3,7,1,0,7,2,8,7,5,3,3,9,0,2,1,0,2,7,9,8,7,9,7,9,9,8,2,2,0,8,3,7,5,9,0,2,4,6,5,1,0,1,3,5,7,4,0,2,5,0},
    ...
    // Variable where we will store the added values of the current digit + remainder from previous addition.
    int buffer = 0;
    // Variables used to store the last 10 digits.
    int index = DIGITS - 1;
    int sum[DIGITS];
    // Add up the digits from each number, starting with the last one.
    for (int x = NR_LENGTH - 1; x >= 0; x--){
        for (int y = 0; y < NR_AMOUNT; y++){
            buffer += numbers[y][x];
        }
        // If we are far enough along that we want to start storing the digits, store them in an array.
        if (x == index){
            sum[index] = buffer % 10;
            index--;
        }
    ...
    // Our buffer may contain additional digits from the adding. We need to get them out of the buffer.
    while (buffer != 0){
    ...
    cout<<"The first "<<DIGITS<<" digits of the sum of these numbers are: ";
